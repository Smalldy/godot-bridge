...
- A Godot 4.x project that registers the `McpInteractionServer` autoload (`mcp_interaction_server.gd` at project root — godot-mcp projects already have this)
- `node` on PATH
...
3. Validate the mount (`agentPresets.standingKeyFor('<your-preset>')`), then start a session on that preset — the six `godot_*` tools are available.
